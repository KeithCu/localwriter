...
- **Bulk CSV / Formula Range Writing**: `write_formula_range` performs bulk programmatic `setDataArray`/`setFormulaArray` updates, and can ingest raw CSV string blocks (parsing delimiters automatically) without file I/O overhead. Description order is values / native formulas / fill-down first, `=PY` spill second (anti-husk `to_pandas()` paragraph stays verbatim). Optional `source` (A1 / `Sheet.A1`) copies that block onto dest `range` (top-left / source extent) via Calc `copyRange` — formulas + values, relative refs adjust; do not pass `values` with `source`. There is no separate `copy_range` tool.
