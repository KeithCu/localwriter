...
| 6 | [`calc-primary-model/`](calc-primary-model/) | Calc-primary model (branch profitability) | Ready | `5f6c57dd-feb6-4e70-b152-4969d92d1608` |
...
Calc-primary model — five schedule sheets) and restores when done.
...
Stubs **5 and 7–10 are not wired** into `--task` / `--launch` / `--score`.
Do not invent helper flags for them. See each stub `run.md`.
...
`$TMP/writeragent-eval2-calc-primary` and opens that copy (in-workbook).
...
Stub rubric outlines live in each remaining stub folder; no CLI scorer yet.
