...
Change Control) and restores when done. Everyday default stays **15**.
...
Stubs **5–9 are not wired** into `--task` / `--launch` / `--score`. Slot
...
Stub rubric outlines live in each `5–9` folder; no CLI scorer yet.
