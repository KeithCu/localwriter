...
* **Paper-form blanks:** Empty / near-empty text-capable shapes get `blank` and, when they are a fill target, `fillable` plus an optional `label_hint` (nearest sibling text to the left, else above). Use the Name with `fill_draw_fields` / `shape_upsert`. This is **not** PDF/AcroForm.
* **ControlShapes:** Live form widgets include `control` (`type`, `name`, current `text` / `state` / `selected`) so the main agent can see them without a forms delegation.
