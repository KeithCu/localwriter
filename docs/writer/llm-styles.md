...
|-----------|--------------|-------------------|
| Whole-paragraph alignment, colour, or margins (not a named style) | Reported on **read** as read-only `data-lo-para` (FODT automatic style; includes `color:` when `fo:color` is set). Ignored on **write** — sending the attribute back does not restore `Para*` | Read `data-lo-para` to tell an indented or coloured quote from body text; apply a named style per range with `apply_style(clear_direct='style_props')` so the style wins over the hand-set values |
