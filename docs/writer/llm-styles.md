...
- **`data-lo-para` (read report):** the same FODT sidecar also lists each automatic style's **own** properties — margins, indent, alignment, line-height, and paragraph-level font/size/weight/style. Those are emitted as `data-lo-para="margin-left:3.25cm; font-size:12pt"`. DO: use that attribute to tell a hand-indented quote from body text before you style it. It is **not** a write instruction — sending it back is ignored (the result lists `ignored_attributes`). Paragraph colour (`fo:color`) is **not** mapped into `data-lo-para`; character-level colour still appears on spans as inline `style`.
...
|-----------|--------------|-------------------|
| Whole-paragraph geometry / font (margins, indent, alignment, line-height, paragraph-level font) | **Read** reports them as read-only `data-lo-para`. **Write** ignores `data-lo-para` (cannot restore `Para*`). | DO: read `data-lo-para` to classify the paragraph. To make a named style win, `apply_style` on that range with `clear_direct='style_props'` — refused on `full_document` because that would flatten the distinctions you just read. |
| Paragraph colour | **Not** in `data-lo-para` (`fo:color` is not mapped). Run colour still appears as span `style`. | DO: read span `style` for coloured runs. To change colour via a style, `style_update` `CharColor`, then `apply_style(..., clear_direct='style_props')` so a hand-set font/size does not hide the style. `style_props` keeps run colour (it is not a style-governed slot). |
| House font (`style_update` + re-apply the same paragraph style) | Re-applying a style the paragraph already has leaves direct `Char*` standing, so the old font stays. The result echoes `preserved_char_overrides` (including Asian/Complex font/size). | DO: re-apply with `clear_direct='style_props'` so font/size fall back to the style; bold/italic/colour survive. |
