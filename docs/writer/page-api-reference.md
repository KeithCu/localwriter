...
### Key Properties
- **`HeaderIsOn`** / **`FooterIsOn`** (`bool`): Enables or disables the header/footer. `page_set_style_properties` with `header_is_on=false` / `footer_is_on=false` **refuses** if any matching region still holds text, fields, images, or tables (LibreOffice’s “delete header?” spirit — no silent disable). F5: disabling the region **clears** its content; the refuse exists so we do not silently wipe. Clear first with `page_set_header_footer_text(region=…, content='')`, then disable. Enabling (`true`) is always allowed. There is no force-off flag.
