...
These variants are exposed as the `region` values of `page_get_header_footer_text` / `page_set_header_footer_text`: `header`, `footer`, `header_first`, `footer_first`, `header_left`, `footer_left`. When the matching `*IsShared` flag is on, the variant mirrors the shared text, so asking for it is always safe. `page_get_style_properties` reports `first_is_shared`, and `page_set_style_properties` writes it. **Turn the header/footer on first** — `FirstIsShared` reads/writes as nothing while `HeaderIsOn` / `FooterIsOn` is false. `style_get_info(family=PageStyles)` returns that same properties payload in-process via `read_page_style_properties` (both public tools stay; no extra hop). `image_insert` uses the same names as `target` for `header`, `footer`, `header_first`, and `footer_first`. A first-page letterhead logo needs `first_is_shared=false` so `HeaderTextFirst` is its own text object; then `target=header_first` (or `footer_first`). Shared `target=header` still writes `HeaderText`, which does not reach the first page when `FirstIsShared` is false.
