...
Host-level unit tests are in `tests/chatbot/test_tool_loop_errors.py`. Mock-sidebar Packet K (`make test-mock-sidebar FILTER=K`) covers proactive compact, overflow retry, kill switch, and death-vs-overflow against `writeragent-mock` (catalog window 32768). URP cases grow `ChatSession` via debug `INFLATE_HISTORY` rather than streaming a 24k HTML flood through the rich control.
