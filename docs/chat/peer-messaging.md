...
- **Inner document_research** (`PEER_INNER_CHOICE_RULES` + catalog): When an Open peers entry matches the file the task is about, Do `send_peer_message` to that peer (not `delegate_read_document`). Why: that sidebar is live and can change the document, run analysis, and use that app's tools; silent reopen only peeks, duplicates work, and races the peer reply. Do `delegate_read_document` only when the file is **not** in Open peers (nearby on disk / no live sidebar). When the task includes a `peer_ask_id` or says reply to a `[Peer from: …]` envelope you **must** `send_peer_message` with that `document_url` and `peer_ask_id` **before** `specialized_workflow_finished`. Why: putting the reply only in `answer` stays inside this loop — the peer sidebar never sees it. HTML / table / other result text in the task **is the `message` argument** (one string; Writer later uses `apply_document_content`), not a final answer. Why: the outer already did the local work; your job is deliver via the tool. Do `send_peer_message` even when you can answer from the task alone — that smol rule is for silent research finishes; peer delivery is a tool side effect. After `ok`/`accepted` you **must** `specialized_workflow_finished` immediately (tool result repeats this). Why: waiting deadlocks the peer.
