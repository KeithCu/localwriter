...
- **Inner document_research** (`PEER_INNER_CHOICE_RULES` + catalog): Do `send_peer_message` when an Open peers entry is listed and that sidebar must change / compute / write / run as an agent. Do `delegate_read_document` for a silent file fact. When the task includes a `peer_ask_id` or says reply to a `[Peer from: …]` envelope you **must** `send_peer_message` with that `document_url` and `peer_ask_id` **before** `specialized_workflow_finished`. Why: putting the reply only in `answer` stays inside this loop — the peer sidebar never sees it. HTML / table / KPI text in the task **is the `message` argument** (one string; Writer later uses `apply_document_content`), not a final answer. Why: the outer already did the local work; your job is deliver via the tool. Do `send_peer_message` even when you can answer from the task alone — that smol rule is for silent research finishes; peer delivery is a tool side effect. After `ok`/`accepted` you **must** `specialized_workflow_finished` immediately (tool result repeats this). Why: waiting deadlocks the peer.
