...
**This experiment (specialized-inner):** outer/main schemas never list `send_peer_message` (even when peers are open). Outer prompt is a thin DO: delegate `document_research` for sibling work. After that inner send is accepted (or the answer says waiting for a peer reply), the **outer Readys** — do not start more `document_research` / python / query tools in the same turn. On a `[Peer from: …]` envelope, do local work; delegate again to send a peer reply **only when the peer asked for work that needs an answer back**. If the envelope is already a data/result reply (a result table or HTML payload to insert), apply locally and stop — no ack specialize. The document_research subagent is the only loop that calls `send_peer_message`.

**Specialize is one-shot (2026-09-09).** `specialized_workflow_finished` is **hidden** from specialized schemas (and from the inner wire list). After `ok`/`accepted` the **host** ends the specialize and returns to the outer loop — do not wait for the model to call finish. If more sibling work is needed, the outer issues another specialize. Waiting in the inner loop still deadlocks the peer (drain owner stays held → pending kick never runs). Ready-after-accepted / drain-kick is unchanged.

**Floorstand headed lesson:** even `google/gemini-3.8-flash` skipped finish after accepted (SIG-A / Packet P2 class failure) — minutes of Thinking, then blank Ready. Prompt + `PEER_ACCEPTED_FINISH_HINT` already demanded immediate finish; the model still skipped. Host enforcement is the lock. MCP stays refused.
...
3. Gateway prepends `[Peer from: Risk memo.odt | uid=… | url=… | peer_ask_id=…]` on the **Calc** session. The host ends the specialize after accepted (one-shot; no finish tool). Writer **Readys** (no further document_research / python / query in that turn). Caller drain exits.
...
5. Calc document_research calls `send_peer_message(document_url=<Writer uid or url from the envelope>, message=<table + ranges>, peer_ask_id=…)`. The host ends that specialize after accepted. Host wraps with Calc’s `ctx.doc` envelope and queues/injects onto **Writer**.
...
2. Writer document_research `send_peer_message(document_url=<stand-in uid>, message="Fill the change-control fields from this discrepancy summary: … Reply with a short confirmation.")` → `{ok, accepted, peer_ask_id}`; host returns to outer. Writer Readys.
...
4. Draw document_research `send_peer_message(document_url=<Writer uid or url from the envelope>, message=<confirmation>, peer_ask_id=…)`; host ends the specialize. Writer follow-up cites the form in the memo.
