...
**Streaming**: Worker thread → `queue.Queue`; first tuple element is [`StreamQueueKind`](plugin/framework/async_stream.py) (legacy string tags still accepted at the drain). Main thread drain loop + **`toolkit.processEventsToIdle()`**. **Do not** use UNO `XTimerListener` in the sidebar for this. [`run_blocking_in_thread`](plugin/framework/async_stream.py) pumps the UI while awaiting a result (`BlockingPumpKind` on its internal queue). **`LlmClient`** cached on sidebar, MainJob, and `prompt_function` for keep-alive. Edge cases: [LITELLM_INTEGRATION.md](LITELLM_INTEGRATION.md). Remaining producers / phased plan: [docs/stream-queue-kind-migration.md](docs/stream-queue-kind-migration.md).
