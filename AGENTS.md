...
- **apply_markdown**: Inserts or replaces content using Markdown **with native formatting**. Parameters: `markdown` (string), `target` (`"beginning"` | `"end"` | `"selection"` | `"search"`); when `target="search"`, also `search`, optional `all_matches`, `case_sensitive`. Implementation: writes markdown to a temp `.md` file, then calls **`cursor.insertDocumentFromURL(file_url, {FilterName: "Markdown"})`** on a text cursor positioned at the target location. LibreOffice's `XDocumentInsertable` interface renders the markdown through its native filter and inserts formatted text (headings, bold, italic, lists, etc.) directly into the document. No hidden document, no transferable, no clipboard needed. See `markdown_support.py`.
