...
def search_engine_preview_line(query_for_engine: str) -> str:
    ...
    return _("This search query '%s' will be sent to the search engine.") % (
        query_for_engine or "",
    )


def web_search_engine_step_chat_text(
    query_for_engine: str, step_index: int, *, approval_required: bool
) -> str:
...
def web_research_engine_chat_block(query_for_engine: str, *, approval_required: bool) -> str:
    ...
    return web_search_engine_step_chat_text(
        query_for_engine, 0, approval_required=approval_required
    )
