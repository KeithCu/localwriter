...
class WebResearchTool(ToolBase):
    ...
    def execute(self, ctx, query, history_text=None):
        ...
        from plugin.contrib.smolagents.default_tools import (
            DuckDuckGoSearchTool,
            VisitWebpageTool,
        )
        from plugin.contrib.smolagents.memory import (
            ActionStep,
            FinalAnswerStep,
            ToolCall,
        )
        ...
        cache_path = (
            os.path.join(udir, "localwriter_web_cache.db")
            if (udir and cache_max_mb > 0)
            else None
        )
        ...
        for step in agent.run(task, stream=True):
            if stop_checker and stop_checker():
                return format_error_payload(
                    ToolExecutionError(
                        "Web search stopped by user.", code="USER_STOPPED"
                    )
                )
            if isinstance(step, ToolCall):
                ...
                if append_thinking_callback:
                    ...
                    if step.model_output:
                        ...
                        getattr(step, "model_output_message", None)
                        and step.model_output_message.content
                    ):
