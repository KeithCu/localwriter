...
- Headers/footers: read/write HTML with page_get_header_footer_text(format='html') and page_set_header_footer_text with that HTML (same XHTML stack as the body; page-number fields are <span title="page-number"/>).
  Plain text on page_set_header_footer_text uses setString and is refused when the region holds logos/fields. target='search' still reaches headers for a short in-place wording change.
